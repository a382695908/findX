...
var mysql_conn = mysql.connect('localhost', 'findXmgr', 'DB_FINDX', 'password', '3306', 'utf8'); 
...
if ( enable_weixin ) {
    ...
    var wx_appid='wxxxxxxxx?????';
    var wx_secret='xxxxxxxxxxxxxxxxxxxxxxxxxxxxx';
    ...
    case '10.144.212.27':  // my aliyun
        wx_appid='wxXXXXXXX???????????????????????';
        wx_secret='??????????????????????????????';
        wx_base_url='http://h53d.?????.com';
        wx_link ='http://h53d.?????.com/';
        wx_img  ='http://img.open.egret.com/game/gameIcon/179/89901/icon_200.png';
        wx_title='大家来 - 找X';
        wx_desc ='猴年大吉,恭喜发财. 快来试试你的手机能玩微信3D游戏不?';
        break;
    case '10.10.123.207':  // ???? server
        wx_appid='wx??????????????';
        wx_secret='??????????????';
        wx_base_url='http://findx.????.com';
        wx_link ='http://findx.????.com/';
        wx_img ='http://findx.????.com/resource/findx.jpg';
        wx_title='大家来 - 找X';
        wx_desc ='猴年大吉,恭喜发财. 快来试试你的手机能玩微信3D游戏不?';
        break;
    default: // any other
